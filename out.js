...
function setupTextarea() {
    ...
    updateCharCountinfo();
    var pdb = findPhrasesInRefText(reftextraw).map(function (sent) { return parsePhrase(sent); });
...
setTimeout(function () {
    if (document.getElementById('ta1'))
        setupTextarea();
    var pplist = findPhrasesInRefText(reftextraw).map(function (sent) { return parsePhrase(sent); });
    console.log(pplist);
}, 1000);
